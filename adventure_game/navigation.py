...
while True:
    ...
    if move_request not in available_actions:
        move_request = input("\nЗа пределы карты нельзя! Выбери направление из доступных: ------------------ \n\n")
    if (move_request.lower() == list(actions.values())[1].lower() and int(direction_request) == 21) or (move_request.lower() == list(actions.values())[0].lower() and int(direction_request) == 31):
        move_request = input("\nТам стена, движение заблокировано. ------------------ \n\n")
